function [S,D,curve] = SolveGroupMMV(G,Y,option)
...
lambda = 0.00001; % Sparse factor, bigger the sparser
...
for i = 1:maxIter
    ...
    D = sum(mean(S'*G'*Y+eps,2))/sum(mean(S'*G'*G*S+eps,2));
    D(D<1) = 1;
    % Compute steps
    ...
    if display
    pause(0.2);
    figure(1);subplot(1,2,1);imagesc(D);
